                    ...
                    .StartAsync(async ctx =>
                    {
                        ...
                        foreach (var folderPath in allFolders)
                        {
                            try
                            {
                                ...
                                {
                                    // Determine which formatters to use (skip manually-edited bookinfo.json)
                                    var effectiveFormatters = new List<IMetadataFormatter>();
                                    foreach (var formatter in activeFormatters)
                                    {
                                        ...
                                        if (formatter is BookOrganizerFormatter &&
                                            await MetadataJsonProcessor.IsManuallyEditedAsync(existingFile))
                                        {
                                            if (verbose)
                                                AnsiConsole.MarkupLine("[yellow]Protected:[/] {0} (source=manual)",
                                                    Path.GetFileName(folderPath));
                                            continue;
                                        }
                                        effectiveFormatters.Add(formatter);
                                    }
